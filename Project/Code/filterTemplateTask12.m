function [xhat, meas] = filterTemplateTask12(calAcc, calGyr, calMag)
...
% visualized in a simple illustration.  If athe orientation estimate
...
% Note that it is not necessary to provide inputs (calAcc, calGyr, calMag).
  ...
  Rw = diag([4.629868e-07 3.921599e-07 4.510415e-07]);
  Ra = diag([7.020605e-05 6.935685e-05 6.346485e-05]);
  g0 = [-0.0191 0.0757 9.8521]';
  upper_bound = 9.81 * 1.2;
  lower_bound = 9.81 * 0.8;
  Rm = diag([9.748587e-03 5.220846e-03 3.074807e-02]);
  m0 = [0 27.1384 -76.3608]';
  alpha = 0.01;
  % initialize the value Lk
  Lk = norm(m0);
  ...
  try
    ...
    while server.status()  % Repeat while data is available
      ...
      if ~any(isnan(acc))  % Acc measurements are available.
        % Do something
        % find the outlier
        if norm(acc) < lower_bound || norm(acc) > upper_bound
